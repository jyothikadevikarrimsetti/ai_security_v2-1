...
class GatewayOrchestrator:
    ...
    async def _run_zones(
        ...
        # 4c. Resource-bounded execution against the target database (dialect-routed)
    ...
    def _detect_hallucination(self, sql: str, filtered_schema: dict) -> dict:
        ...
        # Build allowed columns set from column_scope (col_name -> visibility)
        ...
        # Build set of table aliases from FROM/JOIN ... alias patterns
        aliases: set[str] = set()
        ...
        # Strip EXTRACT(...FROM...) to avoid false positives on FROM keyword
        cleaned_sql = re.sub(r"\bEXTRACT\s*\([^)]*\)", "", sql, flags=re.IGNORECASE)
        ...
        if allowed_columns:
            ...
            # Match alias.column patterns (strip the alias prefix)
            alias_col_pattern = r"\b(?:" + "|".join(re.escape(a) for a in aliases) + r")\.(\w+)\b" if aliases else None
            col_refs: set[str] = set()

            if alias_col_pattern:
                for m in re.finditer(alias_col_pattern, sql, re.IGNORECASE):
                    col_refs.add(m.group(1).lower())
            ...
            if select_m:
                ...
                # Find bare or qualified column names in SELECT
            ...
            where_m = re.search(r"\bWHERE\b(.*?)(?:\bORDER\b|\bGROUP\b|\bLIMIT\b|\bHAVING\b|$)", sql, re.IGNORECASE | re.DOTALL)
            if where_m:
