...
async def _seed_columns_fallback(session) -> int:
    ...
    FALLBACK_COLUMNS = {
        "encounter_summaries": [
            ...
            "encounter_type", "report_month", "total_encounters", "total_admissions",
            "total_discharges", "avg_length_of_stay", "bed_occupancy_rate",
            "mortality_count", "mortality_rate", "readmission_count", "readmission_rate",
            "total_revenue", "avg_revenue_per_encounter", "created_at",
        ],
        "population_health": [
            "record_id", "report_quarter", "region", "age_group", "gender",
            "disease_category", "patient_count", "encounter_count",
            "avg_cost", "avg_los", "mortality_rate", "complication_rate", "created_at",
        ],
        "quality_metrics": [
        ...
        "research_cohorts": [
            "cohort_id", "study_name", "pi_name", "irb_number", "start_date",
...
async def _seed_columns_his_hr_financial(session) -> int:
    ...
    HIS_HR_FINANCIAL_COLUMNS = {
        ...
        "patients": [
            ...
            "insurance_id", "aadhaar_number", "created_at", "updated_at",
        ],
        "encounters": [
            "encounter_id", "patient_id", "encounter_type", "admission_date",
            "discharge_date", "department_id", "facility_id", "attending_physician_id",
            "diagnosis_code", "diagnosis_description", "status", "created_at",
        ],
        "vital_signs": [
            "vital_id", "patient_id", "encounter_id", "recorded_at",
            "temperature", "heart_rate", "blood_pressure_systolic",
            "blood_pressure_diastolic", "respiratory_rate", "oxygen_saturation",
            "weight", "height", "recorded_by",
        ],
        "lab_results": [
            "result_id", "patient_id", "encounter_id", "test_name", "test_code",
            "result_value", "result_unit", "reference_range", "abnormal_flag",
            "ordered_by", "collected_at", "resulted_at", "status",
        ],
        "prescriptions": [
            "prescription_id", "patient_id", "encounter_id", "medication_name",
            "dosage", "frequency", "route", "start_date", "end_date",
            "prescribed_by", "pharmacy_notes", "status",
        ],
        "allergies": [
            "allergy_id", "patient_id", "allergen", "reaction",
            "severity", "onset_date", "reported_by", "verified", "created_at",
        ],
        "appointments": [
            "appointment_id", "patient_id", "provider_id", "department_id",
            "facility_id", "appointment_date", "appointment_time", "duration_minutes",
            "appointment_type", "status", "notes", "created_at",
        ],
        "clinical_notes": [
            "note_id", "patient_id", "encounter_id", "note_type",
            "author_id", "note_text", "created_at", "updated_at",
            "is_signed", "signed_by", "signed_at",
        ],
        "departments": [
            "department_id", "department_name", "facility_id", "department_head",
            "floor", "wing", "is_active", "created_at",
        ],
        "facilities": [
            "facility_id", "facility_name", "facility_type", "address",
            "city", "state", "zip_code", "phone", "is_active", "created_at",
        ],
        "staff_schedules": [
            "schedule_id", "employee_id", "department_id", "facility_id",
            "shift_date", "shift_start", "shift_end", "shift_type",
            "is_on_call", "created_at",
        ],
        "units": [
            "unit_id", "unit_name", "department_id", "facility_id",
            "bed_count", "unit_type", "is_active", "created_at",
        ],

        # ── ApolloHR tables ────────────────────────────────────
        "employees": [
            "employee_id", "first_name", "last_name", "email", "phone",
            "department_id", "facility_id", "job_title", "hire_date",
            "employment_status", "manager_id", "created_at", "updated_at",
        ],
        "payroll": [
            "payroll_id", "employee_id", "pay_period_start", "pay_period_end",
            "base_salary", "overtime_pay", "deductions", "net_pay",
            "bank_account", "payment_date", "status",
        ],
        "leave_records": [
            "leave_id", "employee_id", "leave_type", "start_date", "end_date",
            "total_days", "status", "approved_by", "reason", "created_at",
        ],
        "certifications": [
            "cert_id", "employee_id", "certification_name", "issuing_body",
        ...
        "claims": [
            "claim_id", "patient_id", "encounter_id", "claim_type",
            "total_amount", "insurance_id", "facility_id", "submitted_date",
            "adjudicated_date", "status", "denial_reason", "created_at",
        ],
        "claim_line_items": [
            "line_item_id", "claim_id", "procedure_code", "description",
            "quantity", "unit_price", "total_price", "modifier",
            "service_date", "created_at",
        ],
        "insurance_plans": [
            "plan_id", "plan_name", "payer_name", "plan_type",
            "coverage_start", "coverage_end", "premium", "deductible",
            "copay", "is_active", "created_at",
        ],
        "patient_billing": [
            "billing_id", "patient_id", "encounter_id", "total_charges",
            "insurance_paid", "patient_responsibility", "payment_status",
            "due_date", "created_at",
        ],
        "payer_contracts": [
            "contract_id", "payer_name", "contract_type", "effective_date",
            "termination_date", "reimbursement_rate", "fee_schedule",
            "status", "created_at",
        ],
        "payments": [
            "payment_id", "claim_id", "patient_id", "amount",
            "payment_method", "payment_date", "reference_number",
            "payer_type", "status", "created_at",
        ],
    }
    count = 0
